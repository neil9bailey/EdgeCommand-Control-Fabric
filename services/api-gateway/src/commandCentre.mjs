...
import { buildSensingDashboard, summarizeSensingPresence } from "./sensingPresence.mjs";
  ...
  return [
    {
      ...
      detail: `${mqttEsphomeSummary.mappedDeviceCount} MQTT mapped, ${matterThreadSummary.bindingCount} Matter bindings`,
    ...
    },
    {
      id: "security",
      label: "Security",
      headline: `${securitySummary.enabledProfileCount} profiles guarded`,
      detail: `${securitySummary.onlineDeviceCount}/${securitySummary.securityDeviceCount} security devices online, ${securitySummary.approvalProfileCount} approval-bound profile`,
      status: securitySummary.approvalProfileCount > 0 ? "governed" : "ready",
      metrics: [
    ...
    },
    {
      id: "sensing",
      label: "Sensing",
      headline: `${sensingSummary.occupiedZoneCount}/${sensingSummary.zoneCount} zones occupied`,
      detail: `${sensingSummary.averageCo2Ppm}ppm CO2, ${sensingSummary.privacyStrictZoneCount} strict privacy zone`,
      status: sensingSummary.privacyStrictZoneCount > 0 ? "governed" : "ready",
      metrics: [
        { label: "Zones", value: sensingSummary.zoneCount },
        { label: "Confidence", value: sensingSummary.averageConfidence },
        { label: "Policies", value: sensingSummary.policyCount },
      ],
    },
    {
      id: "approvals",
...
function buildActionQueue({ approvals, lightingDashboard, climateDashboard, securityDashboard, waterDashboard, energyDashboard, sensingDashboard, moduleManifestDashboard, moduleBuilderDashboard, moduleMarketplaceDashboard, moduleCertificationDashboard, mqttEsphomeDashboard, matterThreadDashboard, devices, routes, auditEvents, mcpOrchestrator, kraDashboard, simulationDashboard }) {
  ...
  const securityActions = (securityDashboard?.profiles || [])
    .filter((profile) => profile.status === "enabled")
    .slice(0, 3)
    .map((profile) => ({
  ...
  const sensingActions = (sensingDashboard?.profiles || [])
    .filter((profile) => profile.status === "enabled")
    .slice(0, 3)
    .map((profile) => ({
      id: `sensing-${profile.id}`,
      priority: profile.trafficClass,
      workspaceId: "sensing",
      title: `${profile.name} context ready`,
      owner: "Occupancy And Presence",
      status: profile.requiresApproval ? "approval_required" : "ready",
      detail: `${profile.mode} mode / ${profile.zoneTargets.length} zone target(s)`,
      evidence: [...profile.policies, profile.requiresApproval ? "privacy-review" : "summary-only"],
    }));

  const moduleActions = (moduleManifestDashboard?.flags || [])
    .filter((flag) => ["discoverable", "approval_required", "preview"].includes(flag.state) || flag.readiness.status !== "ready")
    .slice(0, 4)
    .map((flag) => ({
      id: `module-${flag.id}`,
      priority: flag.trafficClass,
      workspaceId: "modules",
      title: `${flag.moduleName} ${flag.readiness.status.replace(/_/g, " ")}`,
      owner: "Module Marketplace",
      status: flag.readiness.status,
      detail: `${flag.state} flag / ${flag.artifacts.length} artifact(s) / ${flag.dependencyStatuses.length} dependency(ies)`,
  ...
  const marketplaceActions = (moduleMarketplaceDashboard?.requests || [])
    .slice(0, 3)
    .map((request) => ({
      id: `marketplace-${request.id}`,
      priority: request.priority,
      workspaceId: "modules",
      title: request.name,
      owner: "Module Marketplace",
      status: request.listing?.status || request.status,
      detail: `${request.moduleId} / ${request.listing?.queueStatus || request.listing?.readiness || request.status}`,
      evidence: [request.requestedBy, request.listing?.requiresApproval ? "approval-required" : "request-ready"],
    }));

  const certificationActions = (moduleCertificationDashboard?.profiles || [])
    .filter((profile) => profile.status !== "passed")
    .slice(0, 3)
    .map((profile) => ({
      id: `certification-${profile.id}`,
      priority: profile.trafficClass,
      workspaceId: "modules",
      title: `${profile.name} ${profile.status.replace(/_/g, " ")}`,
      owner: "Certification Harness",
      status: profile.status,
      detail: `${profile.moduleId} / ${(profile.requiredEvidence || []).length} evidence item(s) / ${profile.testSuites.length} suites`,
  ...
  const matterActions = (matterThreadDashboard?.commandProfiles || [])
    .slice(0, 3)
    .map((command) => ({
      id: `matter-${command.id}`,
      priority: command.trafficClass,
      workspaceId: "modules",
      title: command.name,
      owner: "Matter And Thread",
      status: command.requiresApproval ? "approval_required" : "ready_to_execute",
      detail: `${command.deviceId} / ${command.cluster} / ${command.command}`,
      evidence: [command.bindingId, command.requiresApproval ? "approval-required" : "fabric-ready"],
    }));
  ...
  const primaryActions = [...approvalActions, ...marketplaceActions, ...certificationActions, ...mqttActions, ...matterActions, ...moduleActions, ...buildActions, ...lightingActions, ...climateActions, ...securityActions, ...waterActions, ...energyActions, ...sensingActions, ...mcpActions, ...kraActions, ...simulationActions, ...deviceActions];
...
export function buildCommandCentre({ catalog, deviceRegistry, eventLedger, automationEngine, mcpOrchestrator, kraEngine, simulationLab, approvalWorkflow, lightingScenes, climateHvac, securityAccess, waterManagement, energyManagement, sensingPresence, moduleManifest, moduleBuilder, moduleMarketplace, moduleCertification, mqttEsphome, matterThread, authStatus }) {
  ...
  const securityDashboard = buildSecurityDashboard({
  ...
  const sensingDashboard = buildSensingDashboard({
  ...
  const moduleMarketplaceDashboard = buildModuleMarketplaceDashboard({
    marketplace: resolvedModuleMarketplace,
    catalog,
    manifest: resolvedModuleManifest,
    builder: resolvedModuleBuilder,
  });
  const moduleCertificationDashboard = buildModuleCertificationDashboard({
  ...
  const matterThreadDashboard = buildMatterThreadDashboard({
    adapter: resolvedMatterThread,
    deviceRegistry,
    catalog,
  });
